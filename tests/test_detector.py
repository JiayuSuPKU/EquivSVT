...
class TestPatternDetector(unittest.TestCase):
    ...
    def setUp(self):
        ...
        obs = pd.DataFrame({
            'cell_type': pd.Categorical(['A', 'B', 'C'] * (self.n_obs // 3) + ['A'] * (self.n_obs % 3)),
            'numeric_feature': np.random.randn(self.n_obs),
            'batch': pd.Categorical(['batch1', 'batch2'] * (self.n_obs // 2) + ['batch1'] * (self.n_obs % 2)),
        }, index=[f"Cell_{i}" for i in range(self.n_obs)])
        
        # Create var dataframe
        var = pd.DataFrame(
            index=[f"Gene_{i}" for i in range(self.n_vars)]
        )
        ...
        # Create layers
        layers = {
            'normalized': X + np.random.randn(self.n_obs, self.n_vars) * 0.1
        }
    ...
    def test_build_kernel_from_coordinates_matern(self):
        ...
        detector.build_kernel_from_coordinates(self.coords, method='matern', 
                                         bandwidth=1.0, nu=1.5)
    ...
    def test_build_kernel_from_coordinates_car(self):
        ...
        detector.build_kernel_from_coordinates(self.coords, method='car', 
                                         k_neighbors=8, rho=0.9)
    ...
    def test_build_kernel_from_obsp_moran(self):
        ...
        detector.build_kernel_from_obsp('connectivities', is_distance=False, 
                                   method='moran')
    ...
    def test_build_kernel_from_obsp_car_standardize(self):
        ...
        detector = PatternDetector(self.adata, min_cells=5)
        detector.build_kernel_from_obsp('connectivities', is_distance=False, 
                                   method='car', rho=0.9, standardize=True)
    ...
    def test_run_with_layer(self):
        ...
        with patch('quadsv.statistics.compute_null_params') as mock_compute_null:
            with patch('quadsv.statistics.spatial_q_test') as mock_spatial_q_test:
                ...
                results = detector.compute_qstat(source='var', features=gene_subset, 
                                 layer='normalized', n_jobs=1)

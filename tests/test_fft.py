...
class TestFFTKernelBasics(unittest.TestCase):
    ...
    def test_fft_kernel_init_square_fft2(self):
        ...
        shape = (10, 10)
        kernel = FFTKernel(shape, topology='square', method='gaussian', bandwidth=1.0, fft_solver='fft2')
        assert kernel.ny == 10
        assert kernel.nx == 10
    ...
    def test_fft_kernel_init_square_rfft2(self):
        """Test FFTKernel initialization with square grid."""
        shape = (10, 10)
        kernel = FFTKernel(shape, topology='square', method='gaussian', bandwidth=1.0, fft_solver='rfft2')
        assert kernel.ny == 10
        assert kernel.nx == 10
        assert kernel.spectrum is not None
    ...
    def test_fft_kernel_init_hex(self):
        ...
        shape = (8, 8)
        kernel = FFTKernel(shape, topology='hex', method='matern', bandwidth=1.0, nu=1.5, fft_solver='fft2')
        assert kernel.ny == 8
        assert kernel.nx == 8
...
class TestFFTVsSpatialKernelComparison(unittest.TestCase):
    ...
    def test_gaussian_kernel_eigenvalues_close(self):
        ...
        assert np.mean(rel_error) < 1e-5, f"Gaussian kernel eigenvalues rel error: {np.mean(rel_error)}"
    ...
    def test_matern_kernel_eigenvalues_close(self):
        ...
        assert len(fft_evals) == len(spatial_evals)
        rel_error = np.abs(fft_evals - spatial_evals) / (np.abs(spatial_evals) + 1e-10)
        assert np.mean(rel_error) < 1e-5, f"Matern kernel eigenvalues rel error: {np.mean(rel_error)}"
    ...
    def test_moran_eigenvalues_close(self):
        ...
        assert len(fft_evals) == len(spatial_evals)
        rel_error = np.abs(fft_evals - spatial_evals) / (np.abs(spatial_evals) + 1e-10)
        assert np.mean(rel_error) < 1e-5, f"Moran kernel eigenvalues rel error: {np.mean(rel_error)}"
    ...
    def test_q_statistic_close_on_smooth_data(self):
        ...
        for i in range(3):
            ...
            x_data += amplitude * np.sin(2 * np.pi * (freq_x * np.arange(nx) / nx + freq_y * np.arange(ny)[:, None] / ny) + phase)
    ...
    def test_r_statistic_close_on_smooth_data(self):
        ...
        for i in range(3):
            ...
            phase = np.random.uniform(0, 2 * np.pi)
            amplitude = np.random.randn() * 5.0
            a += amplitude * np.sin(2 * np.pi * (freq_x * np.arange(nx) / nx + freq_y * np.arange(ny)[:, None] / ny) + phase)
